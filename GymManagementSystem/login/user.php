...
if(isset($_POST['login']))
{
    ...
    if($count == 1){ 
        // If they match with our record in DB then set a $_SESSION["user"] = username
        session_start();
        $username=$row['username'];
        $_SESSION["user"] = $username;
        //Redirect them to index.php inside "user" folder 
        header("Location: http://localhost/code/user/index.php");
        exit(); 
    }  
    else
    {  
